package leetcode.One.Thousand.AuthenticationManager;
 ...
 * @Desc:
 * @Author: 泽露
 * @Date: 2023/2/9 3:45 PM
 * @Version: 1.initial version; 2023/2/9 3:45 PM
 */
public class AuthenticationManager {
    ...
    /**
     * [过期时间, List<需要过期的tokenId>]
     */
    Map<String, Integer> liveTokenMap;

    public AuthenticationManager(int timeToLive) {
        this.timeToLive = timeToLive;
        liveTokenMap = new HashMap<>();
    }

    public void generate(String tokenId, int currentTime) {
        int expireTime = currentTime + timeToLive;
        liveTokenMap.put(tokenId, expireTime);
    }

    public void renew(String tokenId, int currentTime) {
        if (liveTokenMap.containsKey(tokenId)) {
            Integer expireTime = liveTokenMap.get(tokenId);
            // 未过期才处理
            if (expireTime > currentTime) {
                generate(tokenId, currentTime);
            } else {
                liveTokenMap.remove(tokenId);
            }
        }
    }
    ...
    public int countUnexpiredTokens(int currentTime) {
        int result = 0;
        for (Map.Entry<String, Integer> entry : liveTokenMap.entrySet()) {
